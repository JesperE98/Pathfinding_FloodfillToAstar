...
public class Pathfinding : MonoBehaviour
{
    ...
    Queue<Tile> Dijkstra(Tile start, Tile goal)
    {
        ...
        {
            Tile curTile = frontier.Dequeue();
            if (curTile == start) break;
            
            foreach(Tile neighbor in _mapGenerator.Neighbors(curTile)) {
                int newCost = costToReachTile[curTile] + neighbor._Cost; // Generates a new variable that stores the cost of moving towards that neighbor.
